    ...
    <>
        <div className="container-album">
            { showAlbumForm &&
                ...
                <input type="text" placeholder="Album Name" value={albumInput} onChange={handleAlbumInput} required/>
                <button id="clear" onClick={clearAlbumInput}>Clear</button>
                <button id="create" onClick={createAlbum}>Create</button>
            </div>
            )
            }
            <div className="title-album">
                <h1>Your albums</h1>
                { showBtn && (
                <button id="add-album" onClick={handleShowForm}>Add album</button>
                )}
                { showAlbumForm && (
                <button id="cancel" onClick={handleShowForm}>Cancel</button>
                )}
            </div>
        </div>
        <div className="album-container">
            {albumsData.map(album=>(
                <Album key={album.id} text={album.text} albumId={album.id} handleBackAndForward={handleBackAndForward}/>
            ))}
        </div>
    </>
  )
